...
$mensaje = '';
if (isset($_GET['ok']) && $_GET['ok'] === 'creado') $mensaje = 'Podcast creado correctamente.';
if (isset($_GET['ok']) && $_GET['ok'] === 'editado') $mensaje = 'Podcast actualizado correctamente.';
if (isset($_GET['ok']) && $_GET['ok'] === 'eliminado') $mensaje = 'Podcast eliminado.';

$podcasts = $pdo->query(
    'SELECT t.*, u.nombres AS publicado_por FROM podcasts t LEFT JOIN usuarios u ON u.id = t.usuario_id ORDER BY t.fecha_publicacion DESC'
)->fetchAll();
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Podcasts · Revista Digital NTEP</title><script>!function(){try{var t=localStorage.getItem("dash26-theme"),e=window.matchMedia("(prefers-color-scheme: dark)").matches;document.documentElement.setAttribute("data-theme",t||(e?"dark":"light"))}catch(t){document.documentElement.setAttribute("data-theme","light")}}()</script><script defer="defer" src="runtime.js"></script><script defer="defer" src="vendor-fullcalendar.js"></script><script defer="defer" src="vendor-chartjs.js"></script><script defer="defer" src="vendors.js"></script><script defer="defer" src="2026.js"></script><link href="style.css" rel="stylesheet"></head>
<body data-active="podcasts" data-crumbs="Revista | Podcasts">
<div class="shell">
    ...
    <main class="content">
      <section class="hero">
        <div class="hero-text">
          ...
          <h1 class="hero-title">Podcasts</h1>
          <p class="hero-sub">Enlaces embebidos (URL embed (Spotify/YouTube)) conectados a la tabla <code>podcasts</code>.</p>
        </div>
        <div class="hero-actions">
          <a class="btn btn--primary" href="podcast_form.php"><svg viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg> Nuevo podcast</a>
        </div>
      </section>
      ...
      <div class="grid">
        <section class="col-12 card">
          <div class="card-head"><div class="card-title-wrap"><h2 class="card-title">Todos (<?= count($podcasts) ?>)</h2></div></div>
          <table class="table">
            ...
            <tbody>
              <?php if (!$podcasts): ?>
                <tr><td colspan="5" style="text-align:center;color:var(--t-muted);padding:24px 0">No hay podcasts todavía.</td></tr>
              <?php else: foreach ($podcasts as $x): ?>
                <tr>
                  ...
                  <td style="text-align:right;white-space:nowrap">
                    <a class="btn btn--ghost" style="padding:4px 10px;font-size:12px" href="podcast_form.php?id=<?= (int)$x['id'] ?>">Editar</a>
                    <a class="btn btn--ghost" style="padding:4px 10px;font-size:12px;color:#b3261e" href="podcast_delete.php?id=<?= (int)$x['id'] ?>" onclick="return confirm('¿Eliminar este registro?');">Eliminar</a>
                  </td>
                </tr>
