...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $video['url_embed']         = normalizarUrlEmbed(trim($_POST['url_embed'] ?? ''));
    ...
    if ($video['titulo'] === '' || $video['url_embed'] === '') {
        ...
        if ($esEdicion) {
            ...
            header('Location: videos.php?ok=editado');
            exit;
        } else {
            $stmt = $pdo->prepare('INSERT INTO videos (titulo, url_embed, fecha_publicacion, usuario_id) VALUES (?,?,?,?)');
            $stmt->execute([$video['titulo'], $video['url_embed'], $video['fecha_publicacion'], $usuarioActual['id']]);
            header('Location: videos.php?ok=creado');
            exit;
        }
    }
} elseif ($esEdicion) {
...
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= $esEdicion ? 'Editar' : 'Nuevo' ?> video · Revista Digital NTEP</title><script>!function(){try{var t=localStorage.getItem("dash26-theme"),e=window.matchMedia("(prefers-color-scheme: dark)").matches;document.documentElement.setAttribute("data-theme",t||(e?"dark":"light"))}catch(t){document.documentElement.setAttribute("data-theme","light")}}()</script><script defer="defer" src="runtime.js"></script><script defer="defer" src="vendor-fullcalendar.js"></script><script defer="defer" src="vendor-chartjs.js"></script><script defer="defer" src="vendors.js"></script><script defer="defer" src="2026.js"></script><link href="style.css" rel="stylesheet"></head>
<body data-active="videos" data-crumbs="Revista | Videos | <?= $esEdicion ? 'Editar' : 'Nuevo' ?>">
<div class="shell">
    ...
    <main class="content">
      <section class="hero">
        <div class="hero-text">
          ...
          <h1 class="hero-title"><?= $esEdicion ? 'Editar video' : 'Nuevo video' ?></h1>
        </div>
      </section>
      ...
      <div class="grid">
        <section class="col-12 card">
          <form method="post" action="video_form.php" style="display:flex;flex-direction:column;gap:16px;max-width:600px">
            <?php if ($esEdicion): ?><input type="hidden" name="id" value="<?= (int) $id ?>"><?php endif; ?>

            <div class="field">
              <label class="field-label" for="titulo">Título</label>
              <input class="input" id="titulo" name="titulo" type="text" required value="<?= htmlspecialchars($video['titulo']) ?>">
            </div>

            <div class="field">
              <label class="field-label" for="url_embed">Link de YouTube</label>
              <input class="input" id="url_embed" name="url_embed" type="url" placeholder="https://www.youtube.com/watch?v=..." required value="<?= htmlspecialchars($video['url_embed']) ?>">
              <small style="color:var(--t-muted)">Pega el link normal del video (o el link "compartir"); se convierte al formato embed automáticamente.</small>
            </div>

            <div class="field">
              <label class="field-label" for="fecha_publicacion">Fecha de publicación</label>
              <input class="input" id="fecha_publicacion" name="fecha_publicacion" type="date" required value="<?= htmlspecialchars(substr($video['fecha_publicacion'], 0, 10)) ?>">
            </div>

            <div style="display:flex;gap:10px">
              <button class="btn btn--primary" type="submit"><?= $esEdicion ? 'Guardar cambios' : 'Crear video' ?></button>
              <a class="btn btn--ghost" href="videos.php">Cancelar</a>
            </div>
          </form>
        </section>
      </div>
